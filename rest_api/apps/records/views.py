...
def TestRecordCreate(request, format=None):
    ...
    try:
        ...
        try:
            ...
            Machine.objects.create(alias='test', machine_secret=secret, state='A',
        ...
        with transaction.atomic():
            ...
            try:
                ...
                TestBranch.objects.create(branch_name=branch_str, is_accept=True).save()
                branch = TestBranch.objects.filter(branch_name__iexact=branch_str, is_accept=True).get()
            # print(branch)
            ...
            for tag, tag_list in pgbench.items():
                ...
                try:
                    ...
                    TestCategory.objects.create(cate_name='cate1', cate_sn=tag, cate_order=1).save()
                    test_cate = TestCategory.objects.get(cate_sn=tag)
